function [ydot,CS,CV] = f4pred(week,theta,ydata)
...
for country = 1:COUNTRY
    ...
    delay(country) =  p(13,country);% 0;%
    ah1(country) =  p(14,country);
    ah2(country) =  p(15,country);
    TC(country) =  p(16,country);
    Texp(country) =  p(17,country);
    o31(country) =  p(18,country);
    Ho1(country) =  p(19,country);
end
...
for day = week(1)*7-5:week(end)*7 %
    if (day+7)>4908
        ...
        a0= 42.0288;a1=7.5647; b1=  -1.5835;a2=-4.7420;b2=-10.4014;w=0.0173 ;
ozone(round(day+7-delay),country) = a0 + a1*cos(round((day-delay))*w) + b1*sin(round((day-delay))*w)+ ...
    ...
    beta_AH = (ah1*AH(round(day+7-delay))^2 + ah2*AH(round(day+7-delay)));
    beta_T = (TC/Temp(round(day+7-delay)))^Texp;
...
Vacc_Sea=7+[45*7:57*7,97*7:109*7,144*7:156*7,197*7:209*7,254*7:266*7,306*7:318*7,357*7:369*7,408*7:420*7,459*7:471*7,511*7:523*7,564*7:576*7];
if ismember(round(day),Vacc_Sea) 
    vc=0.17/91;    
end
%% MODELLING 
newS = -(Lambda + q2.*CV(day-1,:)+q1.*CR(day-1,:) - beta.*CS(day-1,:).*CI(day-1,:).^m - (d+vc).*CS(day-1,:) - seedings);
