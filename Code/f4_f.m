...
for day = week(1)*7-5:week(end)*7 %
    if day>3678
        ...
        daytime = mod(day,365.25) + 1;% 2567;
        AH(round(day+7-delay),country) = mean(AH(round(daytime:365.25:day)));
        Temp(round(day+7-delay),country) = mean(Temp(round(daytime:365.25:day)));
        ozone(round(day+7-delay),country) = mean(ozone(round(daytime:365.25:day)));
    end
...
if day> 3677 
    mu = exp(-p(11,country)*NPI(day));
end
% %mu = exp(-p(11,:).*NPI(round(day)));
...
for weekind = 1:length(week)
    for day = week(weekind)*7-6:week(weekind)*7
        newILI(week(weekind),:) = newILI(week(weekind),:) + newI0(day,:);  
    end
    
    if weekind > length(week) - 52
       A(week(weekind)) = newILI(week(weekind)) * 1000000 * p(6,:)*A(week(weekind)-1).^p(18,:); % forecasting
    end
    
end

% A(week(end-53):week(end-1)) = newILI(week(end-53):week(end-1))*1000000;

p_report = (p(6,:)*A([(week(1)-1):(week(end)-1)]).^(p(18,:)));

ydot=[newILI(week(1):week(end),:)].*p_report;
