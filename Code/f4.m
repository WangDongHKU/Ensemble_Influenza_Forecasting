function [ydot, CS, CV, CR]  = f4(week,theta,ydata)
...
npi0 = 10;
NPI(4749:4779)= 43.75;
NPI(4779:4808)= linspace(43.75, 43.75, 30);
NPI(4809:4907)=npi0;
NPI(4907:end)=0;
NPI(4816:end)=0;
load('AH.mat');
...
for country = 1:COUNTRY
    seedings(country) = p(12,country);
    delay(country) =  3;% 0;%
    ah1(country) =  p(13,country);
    ah2(country) =  p(14,country);
    Texp(country) =  p(15,country);
    o31(country) =  p(16,country);
    Ho1(country) =  p(17,country);
end
...
for day = week(1)*7-5:week(end)*7 %
    if day>3678
        a0 =17.58; a1 =-5.927;b1 =-3.159;w = 0.0172;
        AH(day,country) = a0 + a1*cos(day*w) + b1*sin(day*w);
        a0 =23.88; a1 =-5.285;b1 =-3.812;w = 0.0172;
        Temp(day,country) = a0 + a1*cos(day*w) + b1*sin(day*w);
        a0= 44.04;a1=8.616; b1=-1.77;a2=-7.393;b2=-9.709;w=0.01721;
        ozone(day,country) = a0 + a1*cos(day*w) + b1*sin(day*w)+ ...
        a2*cos(2*day*w) + b2*sin(2*day*w);

%         daytime = mod(day,365.25)+2567;
%         AH(round(day+7-delay),1) = mean(AH(round(daytime:365.25:day)));
%         Temp(round(day+7-delay),1) = mean(Temp(round(daytime:365.25:day)));
%         ozone(round(day+7-delay),1) = mean(ozone(round(daytime:365.25:day)));
    end

    betabase = exp(p(1,:)+p(2,:)*cos(4*pi*day/365.25)+ p(3,:)*sin(4*pi*day/365.25)+p(4,:)*cos(2*pi*day/365.25)+ p(5,:)*sin(2*pi*day/365.25));
    beta_AH = exp(ah1*AH((day+7-delay))^2 + ah2*AH(round(day+7-delay)));
    beta_T = exp(-Texp*Temp((day+7-delay)));
    beta_O = ozone((day+7-delay))^(-o31);
    beta_H = exp(-Ho1*Ho((day+7-delay)));
    beta= betabase* beta_AH *beta_T*beta_O*beta_H;
 
if day> 3677
    mu = exp(-p(11,country)*NPI(round(day)-7));
end
